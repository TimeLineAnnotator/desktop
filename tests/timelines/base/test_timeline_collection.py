...
class TestTimelineCollection:
    def test_posts_timeline_kind_instanced_event(self, tkui, tl_clct, tlui_clct):
        with patch("tilia.timelines.collection.events") as events_mock:
            tl_clct.create_timeline(TimelineKind.HIERARCHY_TIMELINE)
    ...
    def test_posts_timeline_kind_uninstanced_event(self, tl_clct, tlui_clct):
        with patch("tilia.timelines.collection.events") as events_mock:
            tl1 = tl_clct.create_timeline(TimelineKind.HIERARCHY_TIMELINE)

            tl2 = tl_clct.create_timeline(TimelineKind.HIERARCHY_TIMELINE)
