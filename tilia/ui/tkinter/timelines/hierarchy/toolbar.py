...
class HierarchyTimelineToolbar(TimelineToolbar):
    ...
    def __init__(self, parent):
        ...
        self.button_info = [
            (
                "split30",
                lambda: events.post(EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_SPLIT),
                "Split unit at current position (s)",
            ),
            (
                "merge30",
                lambda: events.post(EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_MERGE),
                "Merge units (Shift+m)",
            ),
            (
                "group30",
                lambda: events.post(EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_GROUP),
                "Group units (g)",
            ),
            (
                "lvlup30",
                lambda: events.post(
                    EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_LEVEL_INCREASE, "plus"
                ),
                "Increase level",
            ),
            (
                "lvldwn30",
                lambda: events.post(
                    EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_LEVEL_DECREASE, "minus"
                ),
                "Decrease level",
            ),
            (
            ...
            (
                "delete30",
                lambda: events.post(EventName.HIERARCHY_TOOLBAR_BUTTON_PRESS_DELETE),
                "Delete unit (Delete)",
            ),
            (
                "paste30",
