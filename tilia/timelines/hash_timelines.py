...
def hash_timeline_data_by_kind(kind: TimelineKind, tl_data: dict):
    match kind:
        ...
        case TimelineKind.HIERARCHY_TIMELINE.name:
            return hash_timeline_data(['height', 'is_visible', 'name'], hash_hierarchies_data, tl_data)
        case TimelineKind.MARKER_TIMELINE.name:
            return hash_timeline_data(['height', 'is_visible', 'name'], hash_markers_data, tl_data)
        case _:
            raise NotImplementedError


def hash_timeline_data(hash_attributes: list[str], hash_components_func: Optional[Callable[[dict], str]], timeline_data: dict):
    str_to_hash = '|'
...
def hash_timeline_components(hash_attributes: list[str], sort_func: Callable[[dict], Any], component_data: dict):
