...
- `tilia/media/player/qtplayer.py::wait_for_signal`'s nested `QEventLoop().exec()` was the hazard's root cause for media playback (the old `QtPlayer` class that used it directly has since been removed). **Fixed for `QtAudioPlayer`, `QtVideoPlayer`, and CLI's players** (`tilia/ui/cli/player.py::CLIVideoPlayer` and the `"audio"` mapping in `tilia/ui/cli/ui.py::get_player_class` both now reuse `QtAudioPlayer`): all three share `qtplayer.py`'s `WorkerThreadPlayer`/`EngineWorker`/`EngineRequests` base classes, which run `QMediaPlayer`/`QAudioOutput` on a worker `QThread` (`audio_worker.py`/`video_worker.py` hold only the video-specific `QVideoSink`/frame-relay additions — see `EngineWorker`'s `_extra_worker_*` hooks), blocking the calling thread on a plain `threading.Event` instead of a nested event loop — a raw OS wait can't reenter the GUI thread's dispatcher. Position and (for video) frames are pushed from the worker via `PositionRelay`/`FrameRelay` rather than the GUI thread polling-and-blocking for them, which used to cause real lag under CPU contention. `QtVideoPlayer` additionally can't pair its worker-thread player with the GUI-thread `QVideoWidget` via `setVideoOutput()` directly — that looks fine (no crash, correct state transitions) but silently drops all frame delivery; only manual visual testing catches it. **Lesson: no-crash and correct-state-transitions aren't evidence of correct rendering, and a blocking round-trip belongs on one-shot calls only, never a hot polling loop.**
